...
✅ 阶段 7：纳入全量回归——已完成，**367/367 全通过**（后端 303 + 前端 edit_switch 42 + smoke 22）
```
...
## 7. 文档卡片 hover 预览（H1-H5）✅ 已自动化（test_doc_card_hover.py，5/5 通过）

**背景**：dashboard 公共知识 / 项目视图「知识」的文档卡片新增 hover 交互（2026-08-10/11 实现）：
摘要常显 + hover 下拉展开（正文预览 + 引用数 + 删除按钮）。**该功能是场景矩阵（S1-S26）之后新增的，
原有 42 用例均不覆盖 hover 预览**——"编辑保存后返回主页 hover 预览不更新"（d9659f1）即因无测试漏网。

| # | 场景 | 归属 | 状态 |
|---|---|---|---|
| H1 | hover 卡片 → 正文预览渲染（markdown 解析/分行/截断） | 交互 | ✅ test_h1_preview_renders_parsed_lines |
| H2 | 预览内 ref 引用可点击跳转 | 交互 | ✅ test_h2_ref_link_clickable |
| H3 | 引用行条件显示（有引用显示 N / 0 引用不显示） | 交互 | ✅ test_h3_ref_count_conditional |
| H4 | 编辑保存 → 返回主页 → hover 预览更新 | **编辑保存→数据一致性** | ✅ test_h4_preview_updates_after_save |
| H5 | 删除按钮 hover 出现 → 点击 → delete-doc 模态 | 交互 | ✅ test_h5_delete_button_opens_modal |

**fixture**：`hover_docs`（无空格路径根公共知识文档 hover-ref-a/b/c：A↔B 互引、C 无引用）。
⚠️ 用无空格路径的原因：**含空格路径的 ref 链接 marked 不识别为链接、后端 ref 扫描不解码 %20**
（独立待办，见下）——测试聚焦 hover 功能本身。

**待办（空格路径 ref 支持）**：文档路径含空格（如"项目知识管理平台"）时：
- 前端：`[text](ref:路径含空格)` marked 按 URL 解析失败（空格截断）→ 渲染为原文，预览/阅读态均不可点
- 后端：`ref:([^)\s]+)` 提取截断在空格处；路径用 %20 编码时 ref_status 不解码 → 引用计数 miss
- 正确写法应为 `%20` 编码（renderer.js 已有解码逻辑），需后端 ref 扫描补 `unquote` + 前端确认
