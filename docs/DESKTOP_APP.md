...
**版本号**：统一在 `backend/__version__.py`（当前 0.6.0），发版时更新（`desktop/package.json` 的 version 同步）。

**产物**（`desktop/dist/`）：

| 文件 | 说明 |
|------|------|
| `MyKnowledge-0.6.0-arm64.dmg` | 拖入 Applications 安装（Apple Silicon） |
| `MyKnowledge-0.6.0-arm64-mac.zip` | 解压即用，网络受限时用这个分发 |
